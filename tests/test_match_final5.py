    ...
    for y in range(9):
        for x in range(9):
            ...
            if dr is not None:
                ...
                if best_s < 0.65 and len(sorted_scores) > 1:
                    second_best_d, second_best_s = sorted_scores[1]
                    if (best_s - second_best_s) < 0.1 and knn_pred is not None:
                        if knn_pred == best_d:
                            best_s += 0.1
                        elif knn_pred == second_best_d:
                            second_best_s += 0.1
                        if second_best_s > best_s:
                            best_d, best_s = second_best_d, second_best_s

                # Print debugging info for y in [4, 5, 7]
                if n == '截圖 2026-02-24 11.21.56.png' and y in [4, 5, 7]:
                    print(f"  ({y},{x}) -> best=({best_d}, {best_s:.3f}) knn={knn_pred} dr_clean_shape={dr_clean.shape}")
