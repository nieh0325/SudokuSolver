...
def match_digit_rule(dr, templates):
    ...
    for d, t in templates.items():
        ...
        # 1 has very different aspect ratio typically, but resizing messes it up
        if d == 1:
            dr_solid = np.sum(r_dr == 255) / float(th * tw)
            t_solid = np.sum(t_cropped == 255) / float(th * tw)
            if abs(dr_solid - t_solid) > 0.1:
                score -= 0.3
