...
is_dark = np.median(roi_gray) < 130
block_size = max(31, (h // 100) * 2 + 1)
if block_size % 2 == 0: block_size += 1
tt = cv2.THRESH_BINARY if is_dark else cv2.THRESH_BINARY_INV
th = cv2.adaptiveThreshold(roi_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, tt, block_size, 10)
num, lab, stats, cents = cv2.connectedComponentsWithStats(th)
print(f'is_dark={is_dark}, components={num}')
c=0
